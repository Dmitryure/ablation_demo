...
class RGBExtractor(FeatureExtractor):
    ...
    def extract(self, batch: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(frames_rgb, (list, tuple)) or not frames_rgb:
            raise ValueError("`video_rgb_frames` must be a non-empty sequence of RGB frame arrays.")
        ...
        clip = torch.stack(clip_frames, dim=1).unsqueeze(0)
